(function () {
  ...
  let aiRunning = false;
  let aiContinuationQueued = false;
  let uiLocked = false;
  ...
  function actionCardAllowedForHeroes(card, heroIds) { return Boolean(card?.type === "action" && (!card.hero || heroIds.includes(card.hero)) && (!card.leaderOnly || heroIds.includes(card.leaderOnly))); }
...
function loadSavedGame() {
  ...
  function populateTestLabOptions() {
    ...
    const playerCards = testActionCards(elements.testPlayerCategory.value || "all");
    const aiCards = testActionCards(elements.testAiCategory.value || "all");
  ...
  function choiceEffectHtml(card, ownerIndex, fallback = "点击一张卡牌后，这里会显示完整效果。") {
    ...
    return `${art ? `<img src="${escapeHtml(art)}" alt="${escapeHtml(card.name)}完整卡面">` : ""}<div><p class="eyebrow">${escapeHtml(toneLabel(card.tone))} // ${escapeHtml(kindLabel(card.kind))}</p><h3>${escapeHtml(card.name)}</h3><small>${escapeHtml(metrics.join("　"))}</small><b>${escapeHtml(effectSummary(card, ownerIndex))}</b><p>${escapeHtml(card.text || "暂无额外文字效果。").replace(/\n/g, "<br>")}</p></div>`;
  }
  ...
  async function animateTriggeredEffect(effect, timing = "效果结算") {
    ...
    if (!source?.cardName || (!effect?.draw && !effect?.charge && !effect?.damage)) return;
    ...
    setAnimationScene(`<div class="battle-showcase-scene victory"><p class="scene-kicker">${escapeHtml(timing)}</p><h2>${escapeHtml(ownerName)} 的「${escapeHtml(source.cardName)}」效果触发</h2><div class="battle-showcase-layout role-effect-layout"><div class="battle-showcase-cards role-effect-card">${cardFace}</div><aside class="battle-showcase-detail single-effect"><p class="eyebrow">EFFECT DETAIL</p><section><strong>${escapeHtml(source.text || "角色效果")}</strong><b>${escapeHtml(parts.join(" · "))}</b><p>该效果将先于后续阶段结算。</p></section></aside></div></div>`, "battle-showcase-animation");
    await delay(180);
    elements.animationLayer.classList.add("animating");
    await delay(1450);
    hideAnimationScene();
  ...
  async function animateTurnEndEffects(effects) {
    for (const effect of effects || []) {
      await animateTriggeredEffect(effect, effect.timing || "回合结束");
      if (effect.draw) await animateDraw(effect.playerIndex, effect.draw, `「${effect.cardName}」效果抽牌`);
    }
  }
  ...
  async function animateContest(result) {
    ...
    for (const effect of result.effects || []) {
      if (effect.damage && effect.triggerSource) await animateTriggeredEffect(effect, effect.triggerSource.timing || "战斗结算");
  ...
  async function runAiTurnSafely() {
    try {
      await runAiTurn();
    } catch (error) {
      // 动画或异步回调异常不能让 AI 的追击权永久锁住对局。
      game.log("AI 自动流程异常；本次 AI 追击已自动结束并恢复回合。", "system");
      if (game?.phase === "pursuit" && game.pursuit?.playerIndex === 1) {
        const recovery = game.endPursuit(1);
        if (recovery.ok) {
          await animateTurnEndEffects(recovery.turnEndEffects);
          await animateTurnDraw();
        }
      }
      aiRunning = false;
  ...
  function queueAiContinuation() {
    if (aiContinuationQueued) return;
    aiContinuationQueued = true;
    setTimeout(() => {
      aiContinuationQueued = false;
      if (aiMayAct() && !aiRunning) void runAiTurnSafely();
    }, 0);
  }
  ...
  async function continueAfterContestResolution() {
    ...
    if (!aiMayAct()) return;
    if (aiRunning) return queueAiContinuation();
    await runAiTurnSafely();
  }
  ...
  function renderActionSelectionModal() {
    const actionModes = ["charge-select", "upgrade-hero", "upgrade-card", "switch-select", "battle-select", "pursuit-select"];
    ...
    const bindCards = (cards, multi = false) => elements.actionSelectCards.querySelectorAll("[data-card]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (multi) {
        const required = game.upgradeOptions(0, upgradeHeroIndex).sort((a, b) => b.level - a.level)[0]?.level || 0;
        if (upgradeDiscardUids.includes(uid)) upgradeDiscardUids = upgradeDiscardUids.filter((item) => item !== uid);
...
if (!ai.upgradedThisTurn && plan.upgrade) {
  ...
  async function runAiTurn() {
    ...
    if (game.phase === "pursuit" && game.pursuit?.playerIndex === 1) {
      ...
      } else { await animateAndCommitEffectDamage(contestResult.effect); await animateTurnEndEffects(contestResult.turnEndEffects); await animateTurnDraw(); }
      aiRunning = false;
      uiLocked = false;
      aiThinkingLabel = "AI 行动中";
      render();
